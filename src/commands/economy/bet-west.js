...
module.exports = {
    name: 'bet-west',
    description: "Place a bet on a Western Conference Team for an NBA game.",
    options: [
        {
            name: 'team',
            description: 'Select a Western Conference Team from the given options.',
            type: String,
            type: ApplicationCommandOptionType.String, 
            required: true,
            choices: [
                { name: 'Dallas Mavericks', value: 'DAL'},
                { name: 'Denver Nuggets', value: 'DEN'},
                { name: 'Golden State Warriors', value: 'GSW'},
                { name: 'Houston Rockets', value: 'HOU'},
                { name: 'Los Angeles Clippers', value: 'LAC'},
                { name: 'Los Angeles Lakers', value: 'LAL'},
                { name: 'Memphis Grizzlies', value: 'MEM'},
                { name: 'Minnesota Timberwolves', value: 'MIN'},
                { name: 'New Orleans Pelicans', value: 'NOP'},
                { name: 'Oklahoma City Thunder', value: 'OKC'},
                { name: 'Phoenix Suns', value: 'PHX'},
                { name: 'Portland Trail Blazers', value: 'POR'},
                { name: 'Sacramento Kings', value: 'SAC'},
                { name: 'San Antonio Spurs', value: 'SAS'},
                { name: 'Utah Jazz', value: 'UTA'},
            ],
        },
        {
            name: 'amount',
...
The two teams involved in this game are: [**Home Team**: ${teamFullNames[teamResults.homeTeam]} | **Away Team:** ${teamFullNames[teamResults.awayTeam]}]`
        ...
        if (odds !== undefined) {
            ...
            interaction.editReply(
                `Your **$${betAmount}** bet for the **${teamFullNames[team]}** to *win* against the **${teamFullNames[opponentTeam]}** has been successfully processed! Your **current balance** is now: **$${user.balance}**
...
If you want to **edit or cancel** your bet, please type **/edit-bet** or **/cancel-bet**. 
To cancel or edit your bets, please copy & paste the following **Game ID: ${gameID}**.`
            )
            return
